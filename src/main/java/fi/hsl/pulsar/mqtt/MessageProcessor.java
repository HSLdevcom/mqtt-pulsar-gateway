...
public class MessageProcessor implements IMqttMessageHandler {
    ...
    public MessageProcessor(Config config, PulsarApplication pulsarApp) {
        ...
        UNHEALTHY_MSG_SEND_INTERVAL_SECS =
                config.getInt("application.unhealthyMsgSendIntervalSecs");
        ...
        log.info(
                "Using in-flight alert threshold of {} with monitoring interval of {} messages",
                IN_FLIGHT_ALERT_THRESHOLD,
                MSG_MONITORING_INTERVAL);
        log.info(
                "Using unhealthy message send interval threshold of {} s for health check (-1 = not in use)",
                UNHEALTHY_MSG_SEND_INTERVAL_SECS);
                        ...
                        () -> {
                            while (!Thread.interrupted()) {
                                try {
                                    ...
                                    log.info(
                                            "{} was interrupted, stopping message processing",
    ...
    public void sendMessageFromQueue() throws InterruptedException {
        ...
        if (++msgCounter % MSG_MONITORING_INTERVAL == 0) {
            if (inFlight < 0 || inFlight > IN_FLIGHT_ALERT_THRESHOLD) {
                log.error(
                        "Pulsar insert cannot keep up with the MQTT feed! In flight: {}", inFlight);
            } else {
    ...
    public CompletableFuture<Void> handleMessage(String topic, MqttMessage message) {
        ...
        // This method is invoked by the MQTT client (via our connector), so all events arrive in
        // the
        // same thread
        // https://www.eclipse.org/paho/files/javadoc/org/eclipse/paho/client/mqttv3/MqttCallback.html

        // If manual ack is enabled, MQTT messages will be acknowledged to the broker only after the
        // CompletableFuture is completed (i.e. when message has been sent to Pulsar)
        ...
        if (payload != null) {
            TypedMessageBuilder<byte[]> msgBuilder =
                    producer.newMessage().eventTime(now).value(payload);
            ...
            properties.put(
                    TransitdataProperties.KEY_SOURCE_MESSAGE_TIMESTAMP_MS, String.valueOf(now));
            ...
            log.warn(
                    "Cannot forward Message to Pulsar because (mapped) content is null, payload before mapping: {}",
    ...
    private static class QueuedMessage {
        ...
        private QueuedMessage(
                TypedMessageBuilder<byte[]> message, CompletableFuture<Void> messageSentFuture) {
